class Splam::Rules::BadWords < Splam::Rule
  ...
  def run
    ...
    bad_words |= %w( cialis viagra payday loan jihad affordable ) << "health insurance"
    bad_words |= %w( webcam  free-web-host rapidshare muslim jihoy store.com ) << "air jordan"
    bad_words |= %w( coach prices clothes discount ugg motorcycle bag bags boots shoes outlet gucci forex cheap locksmith)
    bad_words << /pel?cula/ << /pornogr?fica/ << "portal porno" # srsly, spamming in spanish?

    suspicious_words =  %w( coupon free buy galleries dating gallery hard hardcore video homemade celebrity ) << "credit card" << "my friend" << "friend sent me"
    suspicious_words |= %w( adult pharmacy overnight shipping free hot movie nylon arab ?????? xxx) << "sent me a link"
    suspicious_words |= %w( usa jersey nfl classified classifieds disney furniture camera gifts.com mp5 ) << "flash drive"
